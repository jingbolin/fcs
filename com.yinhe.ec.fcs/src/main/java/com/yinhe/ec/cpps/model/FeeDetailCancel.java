// Decompiled by Jad v1.5.8g. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) 
// Source File Name:   FeeDetailCancel.java

package com.yinhe.ec.cpps.model;


public class FeeDetailCancel
{

    public FeeDetailCancel()
    {
    }

    public String getUserId()
    {
        return userId;
    }

    public void setUserId(String userId)
    {
        this.userId = userId;
    }

    public int getCustId()
    {
        return custId;
    }

    public void setCustId(int custId)
    {
        this.custId = custId;
    }

    public String getFeeMonth()
    {
        return feeMonth;
    }

    public void setFeeMonth(String feeMonth)
    {
        this.feeMonth = feeMonth;
    }

    public String getGenDate()
    {
        return genDate;
    }

    public void setGenDate(String genDate)
    {
        this.genDate = genDate;
    }

    public String getMeterNo()
    {
        return meterNo;
    }

    public void setMeterNo(String meterNo)
    {
        this.meterNo = meterNo;
    }

    public String getFeeItemId()
    {
        return feeItemId;
    }

    public void setFeeItemId(String feeItemId)
    {
        this.feeItemId = feeItemId;
    }

    public int getFeeKind()
    {
        return feeKind;
    }

    public void setFeeKind(int feeKind)
    {
        this.feeKind = feeKind;
    }

    public int getDosagePer()
    {
        return dosagePer;
    }

    public void setDosagePer(int dosagePer)
    {
        this.dosagePer = dosagePer;
    }

    public int getPrePay()
    {
        return prePay;
    }

    public void setPrePay(int prePay)
    {
        this.prePay = prePay;
    }

    public Double getTheDosage()
    {
        return theDosage;
    }

    public void setTheDosage(Double theDosage)
    {
        this.theDosage = theDosage;
    }

    public Double getAddDosage()
    {
        return addDosage;
    }

    public void setAddDosage(Double addDosage)
    {
        this.addDosage = addDosage;
    }

    public Double getBaseFee()
    {
        return baseFee;
    }

    public void setBaseFee(Double baseFee)
    {
        this.baseFee = baseFee;
    }

    public Double getAreaFee()
    {
        return areaFee;
    }

    public void setAreaFee(Double areaFee)
    {
        this.areaFee = areaFee;
    }

    public Double getFee()
    {
        return fee;
    }

    public void setFee(Double fee)
    {
        this.fee = fee;
    }

    public Double getFee1()
    {
        return fee1;
    }

    public void setFee1(Double fee1)
    {
        this.fee1 = fee1;
    }

    public Double getFee2()
    {
        return fee2;
    }

    public void setFee2(Double fee2)
    {
        this.fee2 = fee2;
    }

    public Double getFee3()
    {
        return fee3;
    }

    public void setFee3(Double fee3)
    {
        this.fee3 = fee3;
    }

    public Double getFee4()
    {
        return fee4;
    }

    public void setFee4(Double fee4)
    {
        this.fee4 = fee4;
    }

    public Double getdFee1()
    {
        return dFee1;
    }

    public void setdFee1(Double dFee1)
    {
        this.dFee1 = dFee1;
    }

    public Double getdFee2()
    {
        return dFee2;
    }

    public void setdFee2(Double dFee2)
    {
        this.dFee2 = dFee2;
    }

    public Double getdFee3()
    {
        return dFee3;
    }

    public void setdFee3(Double dFee3)
    {
        this.dFee3 = dFee3;
    }

    public Double getdFee4()
    {
        return dFee4;
    }

    public void setdFee4(Double dFee4)
    {
        this.dFee4 = dFee4;
    }

    public Double getFineFee()
    {
        return fineFee;
    }

    public void setFineFee(Double fineFee)
    {
        this.fineFee = fineFee;
    }

    public int getFineFlag()
    {
        return fineFlag;
    }

    public void setFineFlag(int fineFlag)
    {
        this.fineFlag = fineFlag;
    }

    public Double getFineFree()
    {
        return fineFree;
    }

    public void setFineFree(Double fineFree)
    {
        this.fineFree = fineFree;
    }

    public String getFineOper()
    {
        return fineOper;
    }

    public void setFineOper(String fineOper)
    {
        this.fineOper = fineOper;
    }

    public String getFineDate()
    {
        return fineDate;
    }

    public void setFineDate(String fineDate)
    {
        this.fineDate = fineDate;
    }

    public String getFineDesc()
    {
        return fineDesc;
    }

    public void setFineDesc(String fineDesc)
    {
        this.fineDesc = fineDesc;
    }

    public Double getTheFee()
    {
        return theFee;
    }

    public void setTheFee(Double theFee)
    {
        this.theFee = theFee;
    }

    public String getFeeDate()
    {
        return feeDate;
    }

    public void setFeeDate(String feeDate)
    {
        this.feeDate = feeDate;
    }

    public int getPayFlag()
    {
        return payFlag;
    }

    public void setPayFlag(int payFlag)
    {
        this.payFlag = payFlag;
    }

    public int getCheckKind()
    {
        return checkKind;
    }

    public void setCheckKind(int checkKind)
    {
        this.checkKind = checkKind;
    }

    public String getPayNo()
    {
        return payNo;
    }

    public void setPayNo(String payNo)
    {
        this.payNo = payNo;
    }

    public String getRemark()
    {
        return remark;
    }

    public void setRemark(String remark)
    {
        this.remark = remark;
    }

    private String userId;
    private int custId;
    private String feeMonth;
    private String genDate;
    private String meterNo;
    private String feeItemId;
    private int feeKind;
    private int dosagePer;
    private int prePay;
    private Double theDosage;
    private Double addDosage;
    private Double baseFee;
    private Double areaFee;
    private Double fee;
    private Double fee1;
    private Double fee2;
    private Double fee3;
    private Double fee4;
    private Double dFee1;
    private Double dFee2;
    private Double dFee3;
    private Double dFee4;
    private Double fineFee;
    private int fineFlag;
    private Double fineFree;
    private String fineOper;
    private String fineDate;
    private String fineDesc;
    private Double theFee;
    private String feeDate;
    private int payFlag;
    private int checkKind;
    private String payNo;
    private String remark;
}
