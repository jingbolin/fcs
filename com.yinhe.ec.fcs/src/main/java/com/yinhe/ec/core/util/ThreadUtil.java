...
{
	static final Map<String, ExecutorService> EXECUTORS = new ConcurrentHashMap<String, ExecutorService>();
	static final Map<String, List<Future<?>>> FUTURES = new ConcurrentHashMap<String, List<Future<?>>>();
	static final Map<String, Lock> LOCKS = new ConcurrentHashMap<String, Lock>();
	 ...
	 * 线程数
	 * @param seconds
	 * when the number of threads is greater thanthe core, this is the maximum time that excess idle threadswill wait
	...
	public static void execute(String threadName, int core, int seconds, Runnable runnable)
	{
		...
		try
		{
			...
			if (first)
			{
				...
				{
					FUTURES.putIfAbsent(threadName, new ArrayList<Future<?>>());
				}
				else
				{
